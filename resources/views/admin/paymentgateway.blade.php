...
@section('content')
    <div class="card">
        <div class="card-header text-dark"><h2>Pasarela de pagos</h2>
    ...
    <div class="modal" role="dialog" tabindex="-1" id="mdlpaymentgateway">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" id="formpaymentgateway" name="formpaymentgateway">
                        <input type="hidden" id="id" name="id">
                        <div class="form-group">
                            <label for="name">Nombre *</label>
                            <input type="text" class="form-control" id="name" name="name" placeholder="Ingresar su nombre de la pasarela de pagos">
                        </div>
                        <div class="form-group">
...
@section('js')
    ...
    <script>
        ...
        $('#btnAdd').click(function() {
            clearinput();
            $('#modal-title').html('Nueva pasarela de pagos');
            $('#id').val('');
            $('#name').val('');
            $('#state').val(1);
            $('#mdlpaymentgateway').modal('show');
        });
        ...
        $('#btnSave').click(function() {
            if(validate_data() == false)
                return false;
            let data = $('#formpaymentgateway').serialize();
            
            $.ajax({
                url: '{{route('savepaymentgateway')}}',
                type: 'post',
        ...
        $('body').on('click', '.delete', function() {
            ...
            $.ajax({
                    url: 'deletepaymentgateway/' + data['id'],
                    type: 'get',
                    'success': function(response) {
                        if(response == true)
        ...
        $('body').on('click', '.edit', function() {
            clearinput();
            $('#modal-title').html('Editar pasarela de pagos');

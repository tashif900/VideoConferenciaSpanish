    ...
    <div>
        ...
        <div class="body-container">
            <div class="container">
                ...
                <div class="row mb-4">
                    <div class="col-12 col-lg-9" v-if="type == 1">
                    ...
                    <div class="col-12 col-lg-3">
                        <div class="card mt-3">
                            ...
                            <div class="card-body">
                                <div class="media pb-3 border-bottom text-center" v-for="clase in classes" :key="clase.id">
                                    <div class="media-body">
                                        <h6 class="mt-0">{{ clase.name }}</h6>
                                        <!--<button :click="goToCourseClase(clase.course_id, clase.id)" class="btn btn-info btn-sm">Ver Clase</button>-->
                                        <router-link :to="{ path : '/curso/' + clase.course_id + '/' + clase.id}" class="btn btn-info btn-sm">Ver Clase</router-link>
    ...
    export default {
        ...
        methods: {
            ...
            getClassInformation: function() {
                ...
                let clase = this.$route.params.class;

                let config = { headers: { 'Authorization': "Bearer " + this.$store.state.token } }
                var url = '/api/get-course-single';
                axios.post(url,{course: course, clase: clase}, config).then(response => {
                    if (response.status == -9) {
                        this.$router.push('/preview/class/' + response.data.item)
                    }
                   // console.log(response.data, 'Response True')
                    //console.log(response.data.participantes, 'Participantes')
                    this.id = response.data.item2.id;

                    this.getComments(this.id);
                    this.getResources(this.id);

                    this.classes = response.data.item2.classes;
                    this.name = response.data.item.name;
                    this.description = response.data.item.description;
                    this.instructor = response.data.item2.user.name;
                    this.instructorId = response.data.item2.user.id;
                    this.type = response.data.item2.type_course;
            ...
            goToCourseClase(course, clase) {
                alert('Hola');
                //this.$router.push('/curso/' + course + '/' + clase);
            },
            getComments(id) {
                let config = { headers: { 'Authorization': "Bearer " + this.$store.state.token } }
                var url = '/api/get-comments';
                axios.post(url,{course: id}, config).then(response => {
            ...
            storeComment() {
                ...
                axios.post(url,{comment: this.comment, course: this.id}, config).then(response => {
                    if (response.data == -9) {
                        toastr.warning('Ya hizo un comentario para este curso', 'Mensaje del sistema');
                    } else {
            ...
            setRating: function(rating){
                ...
                axios.post(url,{rating: this.rating, course: this.id, user: this.instructorId}, config).then(response => {
            ...
            loadJitsi() {
                ...
                // require("../../external_api.js");

                // var jitsi = new JitsiMeetExternalAPI;
            ...
            meet.on('ready', function() {
                ...
                // if (document.getElementById('video')) return; // was already loaded
                ...
                // var api = jitsi(domain, options);
                // api.executeCommand('displayName', this.$store.state.user.name);
